...
public class BigDouble {
  ...
  public void mult_multN(BigDouble[] array) {
    double mTemp = 1.0;
    int eTemp = 0;
      for(BigDouble d : array)
      {
          mTemp *= d.M;
          eTemp += d.E;
      }
  ...
  public static int normalizeAndSample(Random random, double[] weights) {
      BigDouble[] ar = new BigDouble[weights.length];
      for(int i = 0; i < weights.length; i++)
      {
          ar[i] = fromDouble(weights[i]);
      }
      return normalizeAndSample(random, ar);
  }
